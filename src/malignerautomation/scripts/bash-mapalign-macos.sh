...
function help {
    echo "
	MacOS version uses greadlink instead of readlink.
...
HASMAPSFOFN=false
HASFASTAFOFN=false
if [ -f $MAPSFOFN ]; then HASMAPSFOFN=true; MAPSFOFN=`greadlink -f ${MAPSFOFN}`; else touch maps.fofn; MAPSFOFN=`readlink -f maps.fofn`; fi
if [ -f $FASTAFOFN ]; then HASFASTAFOFN=true; FASTAFOFN=`greadlink -f ${FASTAFOFN}`; fi
